...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        biasActs = [1] + inActs
        weightedSum = self.getWeightedSum(biasActs)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        w1 = 0
        biasActs = [1] + inActs
        for w, input, i in zip(self.weights, biasActs, range(len(self.weights))):
            w1 = w + alpha * delta * input
            self.weights[i] = w1
            totalModification += abs(w1 - w)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            perceptrons = []
            for p in layer:
                perceptrons.append(p.sigmoidActivation(inActs))
            inActs = perceptrons
            output.append(perceptrons)
        return output
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for input, output in examplesTest:
        allLayerOutput = nnet.feedForward(input)
        match = True
        for act in range(len(output)):
            if round(allLayerOutput[-1][act]) != output[act]:
                match = False
        if match == True:
            testCorrect += 1.0
        else:
